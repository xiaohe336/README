...
* **导出数据**: 若要将检测到的位姿（位置、旋转、欧拉角等）保存到 JSON 文件，请添加 `-o` 参数：
  ```bash
  python3 aruco_detector.py -i ./images -o aruco_poses.json
  ```

### 3. 生成打印用的测试码
如果您需要制作物理测试标记，可以使用此命令生成 ArUco 图片并自行打印：
```bash
python3 aruco_detector.py -g
```
* 程序将默认生成 `DICT_5X5_1000` 字典中 ID 为 0-3 的图片。
...
```bash
python3 aruco_detector.py -c -m 0.05 -a DICT_4X4_50
```
